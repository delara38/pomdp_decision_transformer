...
with tqdm(range(episodes)) as tp:
    for i in tp:
        ...
        while not terminate and not truncate:
            ...
            Sp_prime = S_prime[0], S_prime[2]
            done = terminate or truncate

            score+=reward
            #new_data_buffer.add(state_hist, action_hist, reward_hist, A, done)
            cql_data_buffer.add(state_hist[-1*history_lookback:], action_hist[-1*history_lookback:] + [A], reward_hist[-1*history_lookback:]+[reward], Sp_prime, done)
            state_hist.append(Sp_prime)
        ...
        for i in range(1,len(action_hist)):
...
pickle.dump(new_data_buffer,open('expert_data_fully_observing_buffer.pkl','wb'))
pickle.dump(cql_data_buffer,open('cql_data_fully_observing_buffer.pkl','wb'))

np.save("dqn_learning_curve_stats_fully_observing.npz",epr)

expert_agent.save_networks('expert_agent_fully_observing.pt')